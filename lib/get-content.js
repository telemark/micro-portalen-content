...
async function getNews (tag) {
  const url = `${process.env.CONTENT_URL}/wp-json/wp/v2/posts?categories=3&tags=${tag}&filter[orderby]=date&order=desc&nocache=${new Date().getTime()}`
  try {
...
async function getSharedNews () {
  const url = `${process.env.SHARED_CONTENT_URL}/wp-json/wp/v2/posts?categories=30&filter[orderby]=date&order=desc&nocache=${new Date().getTime()}`
  try {
    const { data } = await axios.get(url)
    return data
  } catch (error) {
